            ...
            <div class="mb-4">
              <label class="flex items-center cursor-pointer">
                <input
                  ...
                  class="w-4 h-4 text-gray-900 bg-gray-100 border-gray-300 rounded focus:ring-gray-900 focus:ring-2"
                />
                <span class="ml-2 text-sm font-normal text-gray-700">启用联网搜索</span>
              </label>
              <p class="text-xs text-gray-500 mt-1 ml-6 font-light">
                开启后将结合网络搜索结果生成回答
              </p>
            </div>
...
const useWebSearch = ref(false); // 联网搜索开关
...
// 计算属性：根据选中的知识库动态过滤 RAG 选项
const selectedLibraryInfo = computed(() => {
  if (!selectedLibrary.value) return null;
  return knowledgeLibraries.value.find(lib => lib.id === selectedLibrary.value);
});
...
const ragOptions = computed(() => {
  ...
  // 如果没有选择知识库，只显示"不开启检索"
  if (!library) {
    return [{ value: "no_retrieval", label: "不开启检索" }];
  }
  
  // 如果知识库没有启用图谱，过滤掉"图检索"选项
  if (!library.enable_graph) {
    return baseRagOptions.filter(opt => opt.value !== "graph_only");
  }
  
  // 启用了图谱，显示所有选项
  return baseRagOptions;
});

// 监听知识库变化，自动调整 RAG 模式
watch(selectedLibrary, (newVal) => {
  const library = knowledgeLibraries.value.find(lib => lib.id === newVal);
  
  if (!newVal) {
    // 没有选择知识库，强制设为不检索
    ragMode.value = "no_retrieval";
  } else if (!library?.enable_graph && ragMode.value === "graph_only") {
    // 知识库没有启用图谱，但当前选的是图检索，切换到自动
    ragMode.value = "auto";
  }
});
...
const getRagModeText = (mode) => {
  // 使用 baseRagOptions 而不是 ragOptions，确保所有模式都能找到对应文本
  const option = baseRagOptions.find((opt) => opt.value === mode);
