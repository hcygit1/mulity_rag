...
class RAGNodes:
    ...
    async def web_search_node(self, state: RAGGraphState, runtime: Runtime[RAGContext]) -> RAGGraphState:
        """联网搜索节点

        使用 MCP 进行联网搜索，获取实时信息。
        MCP 调用在独立线程中执行以避免异步上下文问题。
        ...
        """
        self.logger.info("=" * 50)
        self.logger.info("[RAG Graph] 节点: WEB_SEARCH - 联网搜索")
        ...
        if not original_question:
            messages = state.get("messages", [])
            if messages:
                original_question = messages[-1].content if hasattr(messages[-1], 'content') else str(messages[-1])

        if not original_question:
            self.logger.warning("未找到搜索查询，跳过联网搜索")
            state["web_search_results"] = ""
            return state

        self.logger.info(f"联网搜索查询: {original_question}")

        try:
            # 使用独立的搜索服务（在线程池中执行 MCP 调用）
            from backend.service.web_search import web_search
            
            content = await web_search(original_question, max_results=5)
            
            if content:
                state["web_search_results"] = content
                self.logger.info(f"联网搜索成功，结果长度: {len(content)} 字符")
            else:
                self.logger.warning("联网搜索未返回结果")
                state["web_search_results"] = ""

        except Exception as e:
            self.logger.error(f"联网搜索异常: {e}")
            state["web_search_results"] = ""
    ...
    def generate_answer_node(self, state: RAGGraphState, runtime: Runtime[RAGContext]) -> RAGGraphState:
        ...
        基于检索到的文档、联网搜索结果和用户问题，调用LLM生成最终答案。
        ...
        self.logger.info(f"知识库文档数量: {len(retrieved_docs)}")
        self.logger.info(f"联网搜索结果长度: {len(web_search_results)} 字符")
        ...
        try:
            ...
            if web_search_results:
                documents_text += "\n【联网搜索结果】\n"
                documents_text += web_search_results
            
            # 如果两者都没有
            if not retrieved_docs and not web_search_results:
                documents_text = "暂无检索到的相关信息。"
            ...
            try:
                ...
                self.logger.error(f"LLM调用失败: {parse_error}")
    ...
    def route_retrieval_needed(self, state: RAGGraphState, runtime: Runtime[RAGContext]) -> str:
        """路由：是否需要检索（考虑联网搜索）
        ...
        context = runtime.context
        use_web_search = context.use_web_search if context else False
        
        self.logger.info(f"[RAG Graph] 路由检查: need_retrieval={need_retrieval}, use_web_search={use_web_search}")
        ...
        if need_retrieval:
            ...
            # 不需要知识库检索，但需要联网搜索
            self.logger.info("路由决策: 仅联网搜索 -> web_search")
            return "web_search_only"
        else:
            self.logger.info("路由决策: 无需检索 -> direct_answer")
            return "no_retrieval"

    def route_after_retrieval(self, state: RAGGraphState, runtime: Runtime[RAGContext]) -> str:
        """路由：知识库检索后是否需要联网搜索
        ...
        if use_web_search:
            self.logger.info("路由决策: 知识库检索完成，继续联网搜索 -> web_search")
            return "web_search"
        else:
            self.logger.info("路由决策: 知识库检索完成，生成答案 -> generate_answer")
