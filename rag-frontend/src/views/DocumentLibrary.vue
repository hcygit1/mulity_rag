<template>
  <div class="h-screen flex bg-amber-50/30">
            ...
            <div class="flex items-start justify-between">
              <div class="flex-1 min-w-0">
                ...
                >
                  ...
                  <span v-if="library.enable_graph" class="ml-2 px-1.5 py-0.5 bg-amber-100 text-amber-700 rounded text-xs">图谱</span>
                </div>
              </div>
              <div
        ...
        <div class="flex items-center justify-between">
          <div>
            <div class="flex items-center">
              ...
              <span v-if="selectedLibrary.enable_graph" class="ml-2 px-2 py-0.5 bg-amber-100 text-amber-700 rounded text-xs font-normal">
                知识图谱已启用
              </span>
            </div>
        ...
        >
          ...
          >
            <div class="flex items-start justify-between mb-3">
              ...
              <button
                v-if="!selectedLibrary.enable_graph"
              ...
              <span
                v-else
                class="opacity-0 group-hover:opacity-100 text-gray-300 p-1 cursor-not-allowed"
                title="已启用知识图谱，不支持删除单个文档"
              >
                <svg
                  class="w-4 h-4"
                  fill="none"
    ...
    >
      ...
      >
        ...
        >
          ...
          <div v-if="!showEditDialog" class="mb-4">
            <label class="block text-sm font-normal text-gray-700 mb-2"
              >校验码</label
            >
            <input
          ...
          <div v-if="!showEditDialog" class="mb-6">
            <label class="flex items-center cursor-pointer">
              <input
                v-model="libraryForm.enableGraph"
                type="checkbox"
                :disabled="loading"
                class="w-4 h-4 text-gray-900 border-gray-300 rounded focus:ring-gray-900"
              />
              <span class="ml-2 text-sm font-normal text-gray-700">启用知识图谱</span>
            </label>
            <div v-if="libraryForm.enableGraph" class="mt-2 p-3 bg-amber-50 border border-amber-200 rounded-lg">
              <p class="text-xs text-amber-800 font-normal">⚠️ 启用知识图谱后：</p>
              <ul class="text-xs text-amber-700 mt-1 ml-4 list-disc font-light">
                <li>支持实体关系图检索</li>
                <li>文档只能增量添加，无法单独删除</li>
                <li>如需删除，需清空整个知识库</li>
              </ul>
            </div>
          </div>
    ...
    >
      <div
        class="bg-white rounded-xl shadow-[0_4px_16px_rgba(0,0,0,0.12)] border border-gray-100 p-6 w-full max-w-lg mx-4"
      >
        <h3 class="text-lg font-normal text-gray-900 mb-4">添加文档</h3>

        <!-- 图谱提醒 -->
        <div v-if="selectedLibrary?.enable_graph" class="mb-4 p-3 bg-amber-50 border border-amber-200 rounded-lg">
          <p class="text-xs text-amber-800 font-normal">⚠️ 该知识库已启用知识图谱，文档上传后无法单独删除</p>
        </div>
        ...
        <form @submit.prevent="addDocument">
          ...
          <div v-if="uploadTab === 'crawl'">
            ...
            <p class="text-xs text-gray-400 mb-4 font-light">
              💡 网页爬取使用 Markdown 标题分割策略
            </p>
          </div>

          <!-- OSS上传表单 -->
          <div v-if="uploadTab === 'oss'">
            <div class="mb-4">
              <label class="block text-sm font-normal text-gray-700 mb-2"
                >选择文件</label
              >
            ...
            <p class="text-xs text-gray-400 mb-4 font-light">
              💡 OSS上传使用 Markdown 标题分割策略，需要配置 MinerU API
            </p>
          </div>

          <!-- 本地上传表单 -->
          <div v-if="uploadTab === 'local'">
            <div class="mb-4">
              <label class="block text-sm font-normal text-gray-700 mb-2"
                >选择文件</label
              >
            ...
            <div class="mb-6">
              ...
              >
              <select
                v-model="localUploadForm.chunkStrategy"
                :disabled="loading"
                class="w-full px-3 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-gray-900 focus:border-transparent disabled:opacity-50 bg-white"
              >
                <option
...
// 上传方式选项卡: 'crawl' | 'oss' | 'local'
const uploadTab = ref("crawl");
...
const chunkStrategies = [
  { value: "markdown", label: "Markdown 标题分割", desc: "适合技术文档" },
  { value: "recursive", label: "递归分割", desc: "通用文档" },
  { value: "semantic", label: "语义分割", desc: "保持语义完整" },
  { value: "character", label: "固定字符数", desc: "简单场景" },
];
...
const createLibrary = async () => {
  try {
    ...
    const response = await knowledgeAPI.createLibrary({
      ...
      enable_graph: libraryForm.enableGraph,
    });
...
const addDocument = async () => {
  try {
    ...
    // 如果是网站链接类型（爬虫），使用爬取接口
    if (uploadTab.value === "crawl" && documentForm.url) {
      try {
    ...
    // OSS文件上传逻辑
    if (uploadTab.value === "oss" && documentForm.file) {
      try {
        // 1. 获取OSS上传签名URL
        ...
        const ossUrl = uploadResponse.data.split("?")[0]; // 去掉签名参数
        ElMessage.success("文件上传成功，正在解析处理...");

        // 3. 调用后端处理接口（MinerU 解析 + 切块 + 存储）
        const processResponse = await knowledgeAPI.processOSSDocument(
          ossUrl,
          selectedLibrary.value.collection_id,
          documentForm.name,
          selectedLibrary.value.id
        );
        ...
        if (processResponse.status === 200 && processResponse.data?.success) {
          ElMessage.success(
            `文档处理成功，共 ${processResponse.data.total_chunks || 0} 个分块`
          );
          resetDocumentForm();
          showAddDocumentDialog.value = false;
          await selectLibrary(selectedLibrary.value);
          ...
          throw new Error(
            processResponse.data?.error || processResponse.msg || "文档处理失败"
          );
        }
        return;
      } catch (uploadError) {
        console.error("OSS上传处理失败:", uploadError);
        ElMessage.error("OSS上传处理失败: " + uploadError.message);
        return;
      }
    }

    // 其他类型的文档添加（保留原有逻辑作为兜底）
    const documentData = {
...
const handleLocalFileSelect = (event) => {
  const file = event.target.files[0];
  if (file) {
    localUploadForm.file = file;
    if (!localUploadForm.name) {
      localUploadForm.name = file.name;
    }
  }
};

// 本地上传并处理
const uploadLocalDocument = async () => {
  if (!localUploadForm.file || !selectedLibrary.value) {
  ...
  try {
    ...
    // 使用同步服务上传（已包含 MySQL 记录添加）
    const response = await knowledgeAPI.uploadLocalFile(
      localUploadForm.file,
      selectedLibrary.value.collection_id,
      selectedLibrary.value.id,  // 传入 library_id
      localUploadForm.chunkStrategy
    );

    if (response.status === 200 && response.data?.success) {
      ElMessage.success(
        `文档处理成功，共 ${response.data.total_chunks || 0} 个分块`
      );

      // 不再需要单独添加文档记录，同步服务已处理
      resetLocalUploadForm();
      showAddDocumentDialog.value = false;
      await selectLibrary(selectedLibrary.value);
    } else {
      throw new Error(response.data?.error || response.msg || "上传处理失败");
    }
  } catch (err) {
    console.error("本地上传失败:", err);
    ElMessage.error(err.message || "本地上传失败");
  } finally {
    loading.value = false;
  }
};
...
const resetLocalUploadForm = () => {
  localUploadForm.file = null;
  localUploadForm.name = "";
  localUploadForm.chunkStrategy = "markdown";
};
