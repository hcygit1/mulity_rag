...
from backend.config.agent import get_rag_graph_for_collection, NO_COLLECTION_ID
...
from backend.service.conversation_memory import ConversationMemory, format_history_context
...
async def chat_stream(chat_request: ChatRequest) -> AsyncGenerator[Dict[str, Any], None]:
    ...
    try:
        ...
        # 获取 collection_id，如果没有提供则使用无知识库模式
        collection_id = chat_request.collection_id or NO_COLLECTION_ID
        is_no_collection = (collection_id == NO_COLLECTION_ID)
        
        if is_no_collection:
            logger.info("未选择知识库，使用无知识库模式（仅支持直接对话和联网搜索）")
        else:
            logger.info(f"使用 collection_id={collection_id} 处理聊天请求")
        ...
        try:
            memory = ConversationMemory(llm=rag_graph.llm)
            context_data = await memory.get_context(session_id)
            conversation_history = format_history_context(context_data)
            logger.info(f"获取对话历史上下文，消息数: {context_data.get('total_count', 0)}, 使用摘要: {context_data.get('has_summary', False)}")
        except Exception as e:
            logger.warning(f"获取对话记忆失败，继续无历史上下文: {e}")
        
        # 创建 RAG 上下文
        # 无知识库模式：强制设置为不检索
        if is_no_collection:
            retrieval_mode = RetrievalMode.NO_RETRIEVAL
        else:
        ...
        context = RAGContext(
            ...
            use_web_search=chat_request.use_web_search or False,
            ...
            async for mode,chunk in rag_graph.astream(input_data, context, stream_mode="mix"):
                if mode == "updates":
                    ...
                    if node_name == "check_retrieval_needed":
                        ...
                        web_results = node_output.get('web_search_results', '')
                        result_preview = web_results[:200] + "..." if len(web_results) > 200 else web_results
                        content = f"节点名称为{node_name}，联网搜索完成，结果长度: {len(web_results)} 字符"
...
async def get_user_conversations(user_id: str) -> Dict[str, Any]:
    ...
    Returns:
        Dict[str, Any]: 会话列表结果
    """
    return await conversation_service.get_conversations_by_user(user_id)
