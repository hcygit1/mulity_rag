...
async def create_user(username: str, password: str, email: str) -> User:
    """创建用户"""
    logger = get_logger(__name__)
    logger.info(f"create_user开始 - 用户名: {username}, 邮箱: {email}")
    
    # 直接创建session，避免使用单例模式
    session = DatabaseFactory.create_session()
    logger.info("创建session实例成功")
    
    try:
        # 检查用户是否已存在
        logger.info("开始查询现有用户")
        existing_user = session.query(User).filter(
            (User.username == username) | (User.email == email)
        ).first()
        logger.info(f"查询完成 - 找到现有用户: {existing_user is not None}")
...
async def register(register_request: RegisterRequest) -> Response:
    """用户注册"""
    logger = get_logger(__name__)
    logger.info(f"开始注册请求 - 用户名: {register_request.username}, 邮箱: {register_request.email}")
    
    try:
        logger.info("调用create_user函数")
        user = await create_user(
            register_request.username,
            register_request.password,
            register_request.email
        )
        logger.info(f"用户创建成功 - 用户名: {user.username}")
        ...
        logger.error(f"注册失败 - HTTPException: {e.detail}")
        return Response.error(e.detail)
    except Exception as e:
        logger.error(f"注册失败 - Exception: {str(e)}")
