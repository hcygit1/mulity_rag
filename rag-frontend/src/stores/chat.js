...
export const useChatStore = defineStore('chat', () => {
  ...
  const saveConversationToBackend = async (title) => {
    try {
      // 调用后端API创建对话（不再需要传 userId，后端从 token 获取）
  ...
  const sendMessage = async (messageData) => {
    // 解析messageData参数
    let content, ragMode, selectedLibrary, conversationIdOverride, collectionId, maxRetrievalDocs, systemPrompt, useWebSearch
    if (typeof messageData === 'string') {
      ...
      useWebSearch = messageData.useWebSearch  // 联网搜索开关
    } else {
    ...
    try {
      ...
      if (useWebSearch !== undefined) {
        chatData.use_web_search = useWebSearch
      }
