...
class DocumentExtractor:
    ...
    def read_document(self, file_path: str, pdf_extract_method: str = "pypdf2") -> DocumentContent:
        ...
        """
        # 处理带签名参数的 URL，提取真实文件扩展名
        if file_path.startswith(('http://', 'https://')):
            parsed = urlparse(file_path)
            path_without_query = parsed.path  # 去掉查询参数
            file_extension = path_without_query.split('.')[-1].lower()
        else:
        ...
        if file_extension == 'pdf':
            ...
            try:
                ...
                raise Exception(f"TXT文件处理错误: {str(e)}")
